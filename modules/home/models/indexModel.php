...
function get_list_mobile() {
    $list_products = db_fetch_array("SELECT * FROM `tbl_products` WHERE category_product LIKE '%Iphone%' OR category_product LIKE '%Samsung%' OR category_product LIKE '%Oppo%';");
    $result = array(); //Mảng chứa danh sách sản phẩm theo cat_id
...
function get_list_laptop() {
    $list_products = db_fetch_array("SELECT * FROM `tbl_products` WHERE category_product LIKE 'Laptop'");
    $after = array_combine(range(1, count($list_products)), array_values($list_products));
    $result = array(); //Mảng chứa danh sách sản phẩm theo cat_id
    foreach ($after as $item) {
        $item['url'] = "?mod=product&action=detail&id={$item['product_id']}";
...
function get_list_airpot() {
    $list_products = db_fetch_array("SELECT * FROM `tbl_products` WHERE category_product LIKE 'Tai nghe'");
    $after = array_combine(range(1, count($list_products)), array_values($list_products));
    $result = array(); //Mảng chứa danh sách sản phẩm theo cat_id
    foreach ($after as $item) {
        $item['url'] = "?mod=product&action=detail&id={$item['product_id']}";
